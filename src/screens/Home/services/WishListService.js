...
import { getCaterersWish, getTiffinsWish } from '../controllers/WishListController';
...
//   =======REMOVE ALL WISHLIST=======//
export const removeWishListService = async ({dispatch}) => {
	dispatch(startLoader(true))
  try {
    let token = await AsyncStorage.getItem('token');
    let res = await axios.post(
      `${endpoints.baseUrl}user-remove-all-wishlist`,
      null,
      {
        headers: {
          'Content-Type': 'application/x-www-form-urlencoded',
	...
	dispatch(getCaterersWish({limit:100, page:1}));
	dispatch(getTiffinsWish({limit:100,page:1}))
    return res.data;
  } catch (error) {
    if (error.response && error.response.data) {
      throw new Error(error.response.data.message);
